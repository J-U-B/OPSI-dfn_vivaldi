#!/bin/bash
#===========================================================
# Check usage von variables in OPSI scripts (*.ins, *.inc)
# Detect:
...
#
# Version: 1.0
# Jens Boettge <boettge@mpi-halle.mpg.de>  	2016-10-24 11:06:30 +0200
#===========================================================

VARS_DEFINED=$(grep -i -P -e 'def(Var|StringList)' variables.ins  | sed -e 's/^\s+//' -e 's/\s+/ /g' | awk -e '{print $2}' | sort )
VARS_USED=$(cat *.ins *.inc 2>/dev/null| grep -v '^\s*;' | grep -i 'set\s*\$' | sed -e 's/^\s*//' | sed -e 's/\t/ /' | cut -f 2 -d " " | sort | uniq)
